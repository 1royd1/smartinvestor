    ...
    menu = st.sidebar.selectbox("메뉴", ["🏠 홈", "🧾 리포트", "🛡 관리자"] if user["is_admin"] else ["🏠 홈", "🧾 리포트"])
    ...
    if menu == "🏠 홈":
        ...
        def fetch_news():
            ...
            for item in items[:5]:
                ...
                gpt_summary = title
                grok_summary = f"🌐 [Grok] '{title}' 뉴스 요약 (예시)"
...
[원문 보기]({link})"
    ...
    if menu == "🧾 리포트":
        ...
        class PDF(FPDF):
            def header(self): self.cell(0, 10, "추천 리포트", ln=True, align="C")
        ...
        pdf.add_page()
        pdf.body(["TQQQ", "ARKK"])
        path = "/mnt/data/recommend_full.pdf"
        pdf.output(path)
