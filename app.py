            ...
            with st.spinner("추천 종목을 분석 중입니다..."):
                for symbol in DEFAULT_SYMBOLS:
                    try:
                        ...
                        st.warning(f"{symbol} 분석 실패: {str(e)}")
                        continue
                    ...
                    with st.spinner("데이터 분석 중..."):
                        try:
                            ...
                            if hist.empty or len(hist) < 20:
                                st.error(f"'{symbol}'에 대한 데이터를 가져올 수 없습니다. 다음을 확인해주세요:")
                                st.info("1. 올바른 심볼인지 확인 (예: AAPL, MSFT, GOOGL)")
                                st.info("2. 미국 주식은 그대로, 한국 주식은 .KS 또는 .KQ 추가")
                                st.info("3. 네트워크 연결 상태 확인")
                            else:
                            # 기술적 지표 계산
                            hist = calculate_technical_indicators(hist)
